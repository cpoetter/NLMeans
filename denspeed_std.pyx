...
def nlmeans_3d_std(arr, mask=None, mean=None, sigma=None, patch_radius=1,
    ...
    if mean is not None:
        if mean.ndim != 3:
            raise ValueError('mean needs to be a 3D ndarray', mean.shape)
        else:
            mean = np.ascontiguousarray(mean, dtype='f8')
            mean = add_padding_reflection(mean.astype('f8'), block_radius)
    ...
    arrnlm = _nlmeans_3d_std(arr, mask, sigma, mean, patch_radius, block_radius,
                         rician, num_threads)
...
def _nlmeans_3d_std(double[:, :, ::1] arr, double[:, :, ::1] mask,
                double[:, :, ::1] sigma, double[:, :, ::1] mean=None, patch_radius=1, block_radius=5,
        ...
        for i in prange(B, I - B):
            for j in range(B, J - B):
                for k in range(B, K - B):
                    ...
                    out[i, j, k] = process_block(arr, i, j, k, B, P, sigma, mean)
...
cdef double process_block(double [:, :, ::1] arr,
                          ...
                          cnp.npy_intp B, cnp.npy_intp P, double [:, :, ::1] sigma, double[:, :, ::1] mean) nogil:
    ...
    cdef:
        ...
        double summ, d, w, sumw, sum_out, x, sigm, me, sum_mean, non_zero_counter
        double * W
        double * cache
        double * sigma_block
        double denom
    ...
    if mean is not None:
        mean_block = <double *> malloc(BS * BS * BS * sizeof(double))
    ...
    if mean is not None:
        copy_block_3d(mean_block, BS, BS, BS, sigma, i - B, j - B, k - B)
    ...
    for m in range(P, BS - P):
        for n in range(P, BS - P):
            for o in range(P, BS - P):
                ...
                # to summarize standard deviations, we need to summarize the means first
                if mean is not None:
                    me = mean_block[m * BS * BS + n * BS + o]
                    sum_mean += w * me
                    
    # if arr contained standard deviations, add correction factor now
    if mean is not None:
        non_zero_counter = 0
        ...
        for m in range(P, BS - P):
            for n in range(P, BS - P):
                for o in range(P, BS - P):
                    ...
                    me = mean_block[m * BS * BS + n * BS + o]
                    sum_out += w * (me - sum_mean) * (me - sum_mean)

        sum_out = (non_zero_counter - 1.0)/non_zero_counter * sum_out     
